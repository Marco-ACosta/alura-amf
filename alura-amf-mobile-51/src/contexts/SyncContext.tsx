...
export default function SyncContextComponent({ children }: SyncContextProps) {
  ...
  useEffect(() => {
    ...
    const syncInterval = setInterval(async () => {
      await InternetInfo().then((result) => {
  ...
  const syncLocalData = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(
        "Houve um erro ao atualizar os dados locais:",
        (ex as Error).message,
      );
    }
  };
  ...
  const syncCloudData = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(
        "Houve um erro ao atualizar os dados em nuvem:",
        (ex as Error).message,
      );
    }
  };
  ...
  return (
    <SyncContext.Provider
      value={{ isConnectedRef, isSyncingRef, syncData, isDataUpToDate }}
    >
      {children}
    </SyncContext.Provider>
  );
